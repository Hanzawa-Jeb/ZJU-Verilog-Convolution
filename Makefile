...
DIR_UPSTREAM	?= $(DIR_REPO)/sys-project/lab4-1
DIR_BUILD		?= $(CURDIR)/build

DIR_SIM			?= $(CURDIR)/sim
DIR_SRC			?= $(CURDIR)/submit
DIR_INCLUDE		?= $(CURDIR)/include
DIR_INCLUDE_CUSTOM		?= $(DIR_UPSTREAM)/include
...
VERILATOR_FLAGS		:= --cc --exe  --main --timing\
						...
						-CFLAGS "-DVL_DEBUG -DTOP=${VERILATOR_TOP} -std=c++17" -DVL_USER_IEEE_128
...
WSLENV				:=	$(WSLENV):DIR_SRC/p:DIR_SYN/p:DIR_TCL/p:DIR_BUILD/p:DIR_INCLUDE/p:DIR_INCLUDE_CUSTOM/p
# DIR_PROJECT			:=	$(error "please set your vivado's project")
CMD_PREFIX			:=	cmd.exe /c
PATH_TRANS			:=	wslpath -w
endif

export DIR_SRC DIR_SYN DIR_TCL DIR_PROJECT DIR_INCLUDE DIR_INCLUDE_CUSTOM
